...
static void onenet_upload_entry(void *parameter)
{
    ...
    while (1)
    {
        ...
        if (onenet_mqtt_upload_digit("demo_temp", data.data.temp / 10.0f) < 0)
        {
        ...
        {
            LOG_D("buffer : {\"temperature\":%d}", data.data.temp / 10);
        }
        rt_thread_delay(rt_tick_from_millisecond(3 * 1000));
        
        humi_get_data(&data);
        if (onenet_mqtt_upload_digit("demo_humi", data.data.humi / 10.0f) < 0)
        {
            LOG_E("upload has an error, stop uploading");
            break;
        }
        ...
        {
            LOG_D("buffer : {\"humi\":%d}", data.data.humi / 10);
        }
